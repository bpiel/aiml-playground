...
(defn- add-op-to-state*
  [graph-state ^Op op variable-assigments]
  (-> graph-state
      ...
      (update :variable-assigments add-to-variable-assignments variable-assigments)))

(defn add-op-to-state!
  [^Graph {:keys [handle handle-lock state]} ^Op op variable-assigments]
